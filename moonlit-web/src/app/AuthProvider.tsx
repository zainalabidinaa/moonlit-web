...
interface AuthContextType {
  user: any;
  session: any;
  profiles: MoonlitProfile[];
  currentProfile: MoonlitProfile | null;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [profiles, setProfiles] = useState<MoonlitProfile[]>([]);
  const [currentProfile, setCurrentProfile] = useState<MoonlitProfile | null>(null);
  const [addons, setAddons] = useState<AddonManifest[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [guestMode, setGuestMode] = useState(() => {
    try { return localStorage.getItem('moonlit.guestMode') === 'true'; }
    catch { return false; }
  });
  ...
  async function handleSignOut() {
    ...
    setCurrentProfile(null);
    setAddons([]);
    localStorage.removeItem('moonlit.guestMode');
  }
  ...
  return (
    <AuthContext.Provider value={{
      user, session, profiles, currentProfile, addons, isLoading, guestMode,
      signIn: handleSignIn,
      signUp: handleSignUp,
      signOut: handleSignOut,
      resetPassword: handleResetPassword,
      enterGuestMode: handleEnterGuestMode,
