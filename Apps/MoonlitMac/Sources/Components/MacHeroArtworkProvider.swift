...
final class MacHeroArtworkProvider: ObservableObject {
    ...
    /// Mirrors Harbor's hero backdrop selection (`tmdbMovieImages(...)[0]`):
    /// fetch the title's TMDB images, take **backdrops only** (never posters),
    /// sort by `vote_average` descending, and use the highest-rated one.
    private static func heroBackdropURL(metaId: String, type: MediaType, apiKey: String) async throws -> URL? {
        ...
        // for the logo/title overlay, matching how Harbor renders its hero.
        ...
        guard let best = images.backdrops.max(by: { ($0.voteAverage ?? 0) < ($1.voteAverage ?? 0) }) else { return nil }
    ...
    /// `tmdb:tv:123` id (used directly, like Harbor) or an IMDb `tt…` id (via the
    /// TMDB `find` endpoint).
